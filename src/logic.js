...
function getDocument() {
    ...
    var doc;
    var iframe = document.querySelector('iframe.aut-iframe'); // Adjust for Cypress testing
...
function parseQuantityAmountUnit(ItemDescriptionBeforeParsingData) {
    ...
    const match = Array.from(ItemDescriptionBeforeParsingData.matchAll(
...
function getPricePerUnit( priceContainer ) {
    ...
    let price = priceContainer.querySelector(priceItemSelector).innerText.match(/\d+\.\d+/)[0];
...
function injectPricePerUnit( priceItem, pricePerUnit ) {
    ...
    if (pricePerUnitDiv) {
      ...
      pricePerUnitDiv = getDocument().createElement('div');
...
function script() {
        ...
        ( function (){
            ...
            Array.from(
                getDocument().querySelectorAll('div[data-testid][class=""]')
